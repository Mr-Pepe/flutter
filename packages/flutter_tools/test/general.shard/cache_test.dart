...
void main() {
  ...
  group('Cache', () {
    ...
    testWithoutContext('updateAll prints progress header when artifacts need updating', () async {
      final artifact = FakeSecondaryCachedArtifact()..upToDate = false;
      final FileSystem fileSystem = MemoryFileSystem.test();
      final logger = BufferLogger.test();

      final cache = Cache.test(
        fileSystem: fileSystem,
        artifacts: <CachedArtifact>[artifact],
        processManager: FakeProcessManager.any(),
        logger: logger,
      );

      await cache.updateAll(<DevelopmentArtifact>{DevelopmentArtifact.universal});
      expect(logger.statusText, contains('Downloading dependencies...'));
    });

    testWithoutContext(
      'updateAll does not print progress header when all artifacts are up to date',
      () async {
        final artifact = FakeSecondaryCachedArtifact()..upToDate = true;
        final FileSystem fileSystem = MemoryFileSystem.test();
        final logger = BufferLogger.test();
        ...
        await cache.updateAll(<DevelopmentArtifact>{DevelopmentArtifact.universal});
        expect(logger.statusText, isNot(contains('Downloading dependencies...')));
      },
    );
